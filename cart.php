...
if (!isset($_SESSION['user_id'])) {
    // Save the current cart page URL in a session variable
    $_SESSION['redirect_from_cart'] = $_SERVER['REQUEST_URI'];

    // Redirect to the login or registration page
    header("Location: login.php"); // Replace 'login.php' with your desired login or registration page
    exit();
}
                    ...
                    <form action="" method="post">
                        ...
                        <div class="cart">
                            <table class="table table-responsive table-hover table-bordered">
                                ...
                                // Loop through cart items stored in session variables
                                for ($i = 0; $i < count($_SESSION['cart_p_id']); $i++) :
                                    $item = $_SESSION['cart_p_id'][$i];
                                    $size = $_SESSION['cart_size_id'][$i];
                                    $quantity = $_SESSION['cart_p_qty'][$i];
                                    
                                    // Retrieve product details from the database using $item and $size
                                    $statement = $pdo->prepare("SELECT * FROM tbl_product WHERE p_id = ? AND size_id = ?");
                                    $statement->execute([$item, $size]);
                                    $product = $statement->fetch(PDO::FETCH_ASSOC);

                                    $row_total_price = $product['p_current_price'] * $quantity;
                                    $table_total_price += $row_total_price;
                                ?>
                                    <tr>
                                        <td><?php echo $i + 1; ?></td>
                                        <td>
                                            <img src="assets/uploads/<?php echo $product['p_featured_photo']; ?>" alt="">
                                        </td>
                                        ...
                                        <td>
                                            ...
                                            <input type="number" class="input-text qty text" step="1" min="1" max="" name="quantity[]" value="<?php echo $quantity; ?>" title="Qty" size="4" pattern="[0-9]*" inputmode="numeric">
                                        </td>
                                        <td class="text-right">
                                            <?php echo "&#8377;" . $row_total_price; ?>
                                        </td>
                                        <td class="text-center">
                                            <a onclick="return confirmDelete();" href="cart-item-delete.php?id=<?php echo $item; ?>&size=<?php echo $size; ?>&color=<?php echo $color; ?>" class="trash"><i class="fa fa-trash" style="color:red;"></i></a>
                                        </td>
                                    </tr>
                                <?php endfor; ?>
                                <tr>
